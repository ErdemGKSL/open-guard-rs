...
pub fn build_ui(
    ...
    let ignore_btn = serenity::CreateButton::new("config_cp_ignore_private_toggle")
    ...
    let options = vec![
        serenity::CreateSelectMenuOption::new(l10n.t("config-cp-punish-create", None), "create")
            .default_selection(config.punish_when.contains(&"create".to_string())),
        serenity::CreateSelectMenuOption::new(l10n.t("config-cp-punish-update", None), "update")
            .default_selection(config.punish_when.contains(&"update".to_string())),
        serenity::CreateSelectMenuOption::new(l10n.t("config-cp-punish-delete", None), "delete")
            .default_selection(config.punish_when.contains(&"delete".to_string())),
    ];

    let select_menu = serenity::CreateSelectMenu::new(
        "config_cp_punish_when",
        serenity::CreateSelectMenuKind::String {
            options: options.into(),
        },
    )
    .placeholder(l10n.t("config-cp-punish-when-placeholder", None))
    .min_values(0)
    .max_values(3);
...
pub async fn handle_interaction(
    ...
    if custom_id == "config_cp_ignore_private_toggle" {
