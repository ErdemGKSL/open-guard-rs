use crate::db::entities::module_configs::ChannelPermissionProtectionModuleConfig;
...
pub fn build_ui_with_config(
    ...
    let options = vec![
        serenity::CreateSelectMenuOption::new(l10n.t("setup-cpp-punish-create", None), "create")
            .default_selection(config.punish_when.contains(&"create".to_string())),
        serenity::CreateSelectMenuOption::new(l10n.t("setup-cpp-punish-update", None), "update")
            .default_selection(config.punish_when.contains(&"update".to_string())),
        serenity::CreateSelectMenuOption::new(l10n.t("setup-cpp-punish-delete", None), "delete")
            .default_selection(config.punish_when.contains(&"delete".to_string())),
    ];

    let select_menu = serenity::CreateSelectMenu::new(
        format!("setup_module_cpp_punish_when_{}", setup_id),
        serenity::CreateSelectMenuKind::String {
            options: options.into(),
        },
    )
    .placeholder(l10n.t("setup-cpp-punish-when-placeholder", None))
    .min_values(0)
    .max_values(3);
    ...
    let next_button = serenity::CreateButton::new(format!(
        "setup_module_next_{}_ChannelPermissionProtection",
        setup_id
    ))
